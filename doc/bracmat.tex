...
The instruction for reading a file \verb|myprog| and executing its
instructions is \verb|get$myprog|. The \verb|$| is a binary
...
There are a few download options. Get the last revision from
\allowbreak{}\texttt{https://github.com/\allowbreak{}BartJongejan/\allowbreak{}Bracmat}.
...
\subsection{The dummy operator \texttt{\_}}
...
\subsection{Recursion and the \texttt{\_} operator}
...
\begin{quote}
  ...
  \item \bc{TXT}\par
    Present: File is read in ``text'' mode (file mode \verb|r|.) 
    When in combination with \bc{STR}, any zero byte in the input will cause failure. 
    Not present:
    File is read in binary mode (file mode \verb|rb|). However, reading stops at the
    first zero byte! (Only \verb|fil$| can read binary files.)
  \item \bc{BIN}\par
...
Bracmat supports UTF-8 as well as ISO 8859-1 encoded source code. 
Usually it does not matter how characters are encoded, because
...
supported, because those encodings use zero-bytes.
