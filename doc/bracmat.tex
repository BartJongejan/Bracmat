...
chaining multi-facetted webservices into a workflow. In several
...
although defendable, have an appearance that suits you. It is here
...
\item Combine stuff
\item Replace stuff
\item Cope with labyrinthian problems with many culs-de-sac
\end{itemize}
...
You may freely surround binary operators with white-space characters
...
\texttt{https://github.com/ BartJongejan/Bracmat}.
...
_oct{!} She bought a bycicle for Wanja
\end{ex}
% $
...
Patterns may be built up from sub-patterns and may also include
actions that are triggered if a sub-pattern successfully matches (part
of) the subject.
...
the involved binary operators (\verb|$ ' &|) and prefixes
...
same is true for some other operators (\verb,: | _, and
\verb|=|). These operators, too, have a special meaning within
...
etc. are sub-patterns therein. \verb|!a|, \verb|!aa|, etc. stands for
...
\end{v}
Take care for the grouping of the \verb|: &| and \verb,|, operators:
...
operators \verb|.| , whitespace, \verb|+ * ^ \L| and \verb|\D| should
not be escaped normally.
...
White space (operator/cosmetic measure) almost never leads to
confusion. It does in (some) cases where a \emph{nil} leaf without prefixes
is adjacent to the white space operator. For example:
...
\bfun{\bc{= . , | \& :} white space \bc{+ * \caret{} \char`\\L \char`\\D ' \$ \char`\_}}
...
This operator assures that the right hand operator stays
...
Bracmat gives the user practically NO control over the format of
evaluated algebraic expressions, such as the order of terms or
...
The \verb|'| operator with empty lhs is the macro evaluator. The macro
evaluator returns the rhs unchanged, except where the
\verb|$| operator (also with empty lhs) occurs. The expressions headed
by such \verb|$| operators are replaced as follows:

\subsubsection*{\texttt{'\$\$}}

If the rhs is the \verb|$| operator with empty lhs, the macro
evaluator replaces the expression with the rhs of the heading
\verb|$|, and macro-evaluates the rhs of the result.
\begin{ex}
{?} '(one dollar ($($)))
_oct{!} =one dollar ()$
{?} '(two dollars ($($($($)))))
...
\subsubsection*{\texttt{'\$\char`\_}}
\index{dummy operator}

If the rhs is headed by the \verb|_| operator, the expression is
replaced by the rhs, where the dummy operator is evaluated to its
current value.
\begin{ex}[commandchars=&\[\]]
{?} (=a^b):(=?_?) && '(dummy (a_b) power ($(a_b)))
&oct{!} =dummy a_b power a^b
\end{ex}
% $

In all other cases the rhs is evaluated to a variable name, a member
name or a definition. The expression is thereafter replaced by the
value of the variable, the member of the definition. More examples:
\begin{ex}
{?} '(b+a c)
_oct{!} =b+a c
{?} (x=value) & '(a ($x) z)
...
happens if a the function's argument is split into a left subtree and
a right subtree and the function is called with each subtree in turn
as its argument. If the operator in between the subtrees is unknown,
...
evaluated, \emph{expression} is searched for sub-expressions headed by
the operator
\verb|$|, with empty lhs. Such sub-expressions are replaced, depending
...
{?} ((!power : two & (=!arg^2)) | (=!arg^3)) : (=?abc)  _coma[if _bc[power] = _bc[two],]
    _comc[_bc[abc] is bound to _bc[!arg^2] (unevaluated); otherwise, \bc[abc] is bound to _bc[!arg^3]]
...
\verb|=| operator as the member's name, preceded by the objects
...
member has two sub-members \verb|first| and \verb|family|:
\begin{ex}
...
operator are used, but any operator but the \verb|=| operator can be
...
It is also possible to create an alias for a sub-object. Taking the
...
Using an alias for a sub-object can save some code and processing time
if the sub-object is accessed many times. Without the alias for John's
...
Here, \verb|a| and \verb|b| are fields in a nameless sub-object of
...
\begin{ex}
{?} '_dollar(x.):(=?sub-object)
{?} lst_dollar[]sub-object
_oct(sub-object=
_oct(a=1) (b=2));
\end{ex}
...
learned that lists (of lists (of lists\ldots)) lead to smaller and
...
structuring operators, comma and white space.
...
is replaced by the binding of \emph{object name}\verb|.|\emph{member
  name}.
...
Below, use the term `trivial elements' for elements that either are
...
sums, 0 (zero) is the neutral element. In products, 1 is the neutral
element. So the empty string, 0 and 1 are trivial elements in lists,
sums and products, respectively. In addition, 0 is also a trivial
element in products. Lists and sums do not have `zeros'.
...
\item \verb|`|\par
  ...
  or words (white-space operator). In non-sophisticated patterns,
...
should not be the last sub-pattern or precede a subpattern that is
...
backtracking from following sub-patterns. Bracmat may be optimized to
...
strings. Some special characters have to be preceded by a back slash:
\begin{description}
\item  \verb|\a|\par
  attention (bell)
\item \verb|\b|\par
...
If you precede a string with the prefix \verb|@|, then back slashes
...
\item the match evaluator, which handles the unification of pattern
  expressions with subject expressions,
\item the macro evaluator, which merely substitutes certain parts of
  an expression,
\item the archivist, which doesn't do anything but keeping expressions
  alive.
\end{enumerate}
...
the first \verb|?| and insert an output action after each sub-pattern.
\begin{v}
(De kok snijdt recht en de meid snijdt scheef
...
variables. A function is called by \emph{function
  name}\verb|$|\emph{argument expression} or \emph{function
  name}\texttt'\emph{argument expression}, depending on whether
argument expression must be evaluated (\verb|$|) or not (\verb|'|).
...
Both array name and index should evaluate to atoms. \emph{array name}
...
{?} a-value : 2 _d ?array     _com(_bc(array[2]) := _bc(a-value))
{?} array = another-value    _com(_bc(array[2]) := _bc(another-value))
{?} !array : -1 _d ?array     _com(_bc(array[3]) := _bc(another-value))
...
\bfun{\bc{fil\d(}[[\bi{file name}][\bc,\bi{option}[\bc,\bi{number}[\bc,value to output]]]]\bc{)}}
\index{fil@\bc{fil}}
...
{?} fil$("mytext.txt","rb")      _com[open for reading in binary mode]
{?} fil$(,STR,"\n \t\r")         _com[prepare for reading until the next white space]
...
\begin{quote}
  ...
  \item \bc{TXT}\par
    Present: File is read in `text' mode (file mode r.) Not present:
    File is read in binary mode (file mode rb).
  \item \bc{BIN}\par
  ...
  \item \bc{TRM} (together with \bc{ML})\par
    Present: Heading and trailing white space is cut down to a single
    space in character data. Not present: All white space is
...
option is not used, input is coming from standard input. Take care for
putting filenames in double quotes if they contain any characters that
can be misunderstood, such as dots, (back) slashes or dollars.

\bfun{\bc{lst\d(}(\bc=\bi{expression}) or \bi{variable}*[\bc{,LIN}][\bc{,RAW}]\bc)}
\index{lst@\bc{lst}}
...
\bfun{\bc{lst\d(}(\bc=\bi{expression}) or \bi{variable}*[\bc{,MEM}][\bc{,LIN}][\bc{,RAW}]\bc)}
...
is the return value of the call to lst. It has the opposite effect of
\verb|get$(|\bi{atom}\verb|,MEM)|. Use \verb|:| compression of an
expression to save space. If the compressed expression is needed, it
is decompressed with \verb|get$|. Compression is typically by a factor
of about 5, but may be as large as 16. Expressions with very large
atoms (such as this help function) do less well.
...
\bfun{\bc{lst\d(}(\bc=\bi{expression}) or \bi{variable}*\bc,\bi{file name}\bc,\bc{NEW}$\vert$\bc{APP}[\bc{,LIN}][\bc{,TXT}][\bc{,BIN}]\bc)}
...
\bi{fnc} to each member in list. \bi{fnc} can be the name of a
...
\verb|mod$| divides \bi{number} by \bi{divisor}. The rest is returned.
\begin{ex}
{?} mod_d(22,7)
_oct{!} 1
\end{ex}
...
\bfun{\bc{put\d(}\bi{expression}[\bc{,LIN}]\bc)}
\index{put@\bc{put}}
...
\bfun{\bc{put\d(}\bi{expression}\bc{,MEM}[\bc{,LIN}]\bc)}

\bi{expression} is stringized and placed into an atom, which is the
...
\bfun{\bc{put\d(}\bi{expression}\bc,\bi{file name}\bc{,NEW}$\vert$\bc{APP}[\bc{,LIN}]\bc)}
...
\bfun{\bc{sys\d}\bi{command line commando}}
\index{sys@\bc{sys}}
...
_oct{!} (Eccentricity.934/10000) + (moons.,(.Deimos) (.Phobos))
_oct    + (planet..Mars)
\end{ex}
...
_oct{!} (Eccentricity.934/10000) + (moons.,(.Deimos) (.Phobos))
_oct    + (planet..Mars) + (ratio.3711/9807)
\end{ex}
...
\subsubsection*{Mapping between JSON and Brachmat expressions}
...
specified by its name or by its function body. The forall method
finishes when all elements are traversed or before that if the
function fails. The behaviour of forall is undefined if the hash table
is changed or deleted during the traversal, although this can be done
safely. For example may some members be missed and others be processed
more than once.
